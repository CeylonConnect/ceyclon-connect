...
function RatingBadge({ value, count }) {
  ...
  if (!Number.isFinite(safeCount) || safeCount <= 0) {
    return (
      <div className="inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1 text-sm font-semibold text-neutral-800 shadow dark:bg-black/80 dark:text-neutral-100 dark:ring-1 dark:ring-white/10">
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1 text-sm font-semibold text-neutral-800 shadow dark:bg-black/80 dark:text-neutral-100 dark:ring-1 dark:ring-white/10">
      ...
      <span>{(Number.isFinite(safeValue) ? safeValue : 0).toFixed(1)}</span>
...
function CategoryChip({ label }) {
  return (
    <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-neutral-700 shadow dark:bg-black/80 dark:text-neutral-200 dark:ring-1 dark:ring-white/10">
      {label}
    </span>
  );
}
...
function Meta({ icon, children }) {
  return (
    <div className="inline-flex items-center gap-1.5 text-sm text-neutral-600 dark:text-neutral-300">
...
function ExperienceCard({ item }) {
  return (
    <article className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-neutral-800 dark:bg-black">
      ...
      <div className="p-5 sm:p-6">
        ...
        <p
          className="mt-2 text-[15px] text-neutral-600 dark:text-neutral-300"
          style={{
            display: "-webkit-box",
            WebkitLineClamp: 2,
        ...
        <div className="mt-4 flex items-center gap-3">
          <img
            src={item.guide.avatar}
            alt={item.guide.name}
            className="h-9 w-9 rounded-full object-cover"
            loading="lazy"
          />
          <div className="text-sm font-medium text-neutral-800 flex items-center gap-1 dark:text-neutral-100">
            {item.guide.name}
            {item.guide.verified && (
              <span
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="text-xl font-bold text-orange-600">
            {String(item.currency || "").trim() === "$" || !item.currency
              ? "Rs. "
              : item.currency}
            {item.price}
          ...
          <a
            ...
            className="rounded-xl bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:brightness-105 active:scale-95"
          >
            View Details
          </a>
        </div>
